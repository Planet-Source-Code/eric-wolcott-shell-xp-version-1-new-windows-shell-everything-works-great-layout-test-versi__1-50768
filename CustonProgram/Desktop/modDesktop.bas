...
pth = App.path & "\desktop\"
ff = FreeFile
Open App.path & "\desktop.cfg" For Input As #ff
Do Until EOF(ff)
    Line Input #ff, l
    p = InStr(1, l, ",")
    iname(i) = Left(l, p - 1)
    ...
    If Right(frmMain.File1.List(i), 4) = ".lnk" Or Right(frmMain.File1.List(i), 4) = ".esl" Then
        ...
        Else
                ...
                If UCase(Left(icon, 4)) <> "APP," Then
                    icon = Replace(LCase(icon), "%root%", App.path) 'frmMain.ERoot)
                MsgBox icon
...
Open App.path & "\desktop.cfg" For Output As #ff
